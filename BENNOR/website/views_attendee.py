...
def join_room_view():
    ...
    session.clear()
    attendee = Events15.query.all()  # Query all events
    if request.method == "POST":
...
def attendee_invites():
    ...
    # Get the invites stored in the Attendee_events7 table
...
def accept_invite():
    ...
    # Find the event in the Events15 table
    event = Events15.query.filter_by(event_name=event_name).first()
    
    if event:
        # Add this event to the current user's RSVP events in Attendee_events7
        attendee_event = Attendee_events7.query.filter_by(user_id=current_user.id).first()
        if attendee_event:
            ...
            # Save the event in Attendee_records3 when attendee accepts
            new_attendee_record = Attendee_records3(
                event_name=event.event_name,
                event_desc=event.event_desc,
                event_privacy=event.event_privacy,
            ...
            new_attendee_event = Attendee_events7(
        ...
        # Add the user's details to the event's RSVP attendees in Events15
...
def attendee_history():
    ...
    attendee_history = Attendee_records3.query.filter_by(attendee_id=current_user.id).all()
